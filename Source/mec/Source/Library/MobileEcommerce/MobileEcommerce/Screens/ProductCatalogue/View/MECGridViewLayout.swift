...
class MECGridViewLayout: UICollectionViewLayout {

    let numberOfColumns = 2
    let cellHeight: CGFloat = 310
    let cellPadding = CGPoint(x: 1, y: 1)
    ...
    override func prepare() {
        ...
        if let inCollectionView = collectionView {
            ...
            for item in 0 ..< inCollectionView.numberOfItems(inSection: 0) {
                ...
                let frame = CGRect(x: xOffsetList[column], y: yOffsetList[column],
                                   width: inCollectionView.bounds.width,
                                   height: bannerViewHeight)
                let attributes = UICollectionViewLayoutAttributes(forCellWith: indexPath)
                attributes.frame = frame
                layoutAttributesFirstSection.append(attributes)
                yOffsetList[0] = yOffsetList[column] + bannerViewHeight
                yOffsetList[1] = yOffsetList[0]
            }
            ...
            for item in 0 ..< inCollectionView.numberOfItems(inSection: 1) {
                ...
                let height = cellPadding.y + cellHeight
                let frame = CGRect(x: xOffsetList[column], y: yOffsetList[column], width: columnWidth, height: height)
