...
class ECSUpdateAddressRequestTests: XCTestCase {
    ...
    func testUpdateAddressRequestMethod() {
        ...
        let testMicro = ECSEditAddressMicroService()
        testMicro.editSavedAddress(address: addressObject()) { (_, _) in }
        ...
        XCTAssertEqual(ECSTestUtility.stripQueryParametersFor(url: url)?.absoluteString,
                       String(format:"%@/pilcommercewebservices/v2/%@/users/current/addresses/%@", mockBaseURL, mockSiteID, addressId))
        
        XCTAssertEqual(queryParams?.count, 1)
        XCTAssertEqual(queryParams["lang"] as? String, "en_US")
    }
    
    func testUpdateAddressRequestMethodError() {
        mockRestClient.responseData = ECSTestUtility.getResponseFrom(jsonFile: "testSetDeliveryAddressSuccess")
        mockAppInfra.restClient = mockRestClient
