...
class ECSFetchDeliveryModeRequestTests: XCTestCase {
    ...
    func testFetchDeliveryMode() {
        ...
        let testMicro = ECSFetchDeliveryModesMicroService()
        testMicro.fetchDeliveryModes { (_, _) in }
        ...
        XCTAssertEqual(ECSTestUtility.stripQueryParametersFor(url: url)?.absoluteString,
                       String(format:"%@/pilcommercewebservices/v2/%@/users/current/carts/current/deliverymodes", mockBaseURL, mockSiteID))
    }
}
