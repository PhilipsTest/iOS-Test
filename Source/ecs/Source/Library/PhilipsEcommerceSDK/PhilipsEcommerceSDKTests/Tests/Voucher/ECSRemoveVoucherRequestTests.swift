...
class ECSRemoveVoucherRequestTests: XCTestCase {
    ...
    func testRemoveVoucherRequest() {
        ...
        let testMicro = ECSRemoveVoucherMicroServices()
        testMicro.removeVoucher(voucherId: mockVoucherID) { (_, _) in }
        ...
        XCTAssertEqual(ECSTestUtility.stripQueryParametersFor(url: url)?.absoluteString, String(format: "%@/pilcommercewebservices/v2/%@/users/%@/carts/%@/vouchers/%@", mockBaseURL, mockSiteID, currentValue, currentValue, mockVoucherID))
    ...
    func testRemoveVoucherRequestWithError() {
        ECSConfiguration.shared.locale = mockLocale
        ECSConfiguration.shared.rootCategory = mockRootCategory
        ECSConfiguration.shared.siteId = mockSiteID
        ECSConfiguration.shared.hybrisToken = mockAccessToken
