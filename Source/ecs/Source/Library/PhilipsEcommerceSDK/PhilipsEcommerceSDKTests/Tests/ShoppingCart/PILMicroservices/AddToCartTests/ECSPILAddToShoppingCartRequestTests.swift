...
class ECSPILAddToShoppingCartRequestTests: XCTestCase {
    ...
    func testAddToCartRequest() {
        ...
        serviceDiscoveryMock.microserviceURL = "https://acc.eu-west-1.api.philips.com/commerce-service/cart?siteId=\(ECSConfiguration.shared.siteId ?? "")&language=\(ECSConfiguration.shared.language ?? "")&country=\(ECSConfiguration.shared.country ?? "")&quantity=\(quantity)&productId=\(mockProductCTN)&cartId=current"
        
        let testMicro = ECSAddShoppingCartMicroServices()
        testMicro.addECSProductToShoppingCart(ctn: mockProductCTN, quantity: quantity) { (cart, error) in }
        ...
        XCTAssertEqual(queryParams["cartId"] as? String, "current")
    }
    
    func testAddToCartRequestWithoutOauth() {
        mockRestClient.responseData = ECSTestUtility.getResponseFrom(jsonFile: "FetchCartUS")
        mockAppInfra.restClient = mockRestClient
        mockAppInfra.serviceDiscovery = serviceDiscoveryMock
        ECSConfiguration.shared.apiKey = "TestAPIKey"
    ...
    func testAddToCartServiceDiscoveryEntries() {
        mockRestClient.responseData = ECSTestUtility.getResponseFrom(jsonFile: "FetchCartUS")
        mockAppInfra.restClient = mockRestClient
        mockAppInfra.serviceDiscovery = serviceDiscoveryMock
        ECSConfiguration.shared.apiKey = "TestAPIKey"
        ...
        let testMicro = ECSAddShoppingCartMicroServices()
        testMicro.addECSProductToShoppingCart(ctn: mockProductCTN, quantity: quantity) { (_, _) in }
        
        XCTAssertEqual(serviceDiscoveryMock.pilServiceIds?.count, 1)
        XCTAssertEqual(serviceDiscoveryMock.pilServiceIds?.first as? String, "ecs.addToCart")
        ...
        XCTAssertEqual(serviceDiscoveryMock.pilReplacementDict?["cartId"] as? String, "current")
    }
}
