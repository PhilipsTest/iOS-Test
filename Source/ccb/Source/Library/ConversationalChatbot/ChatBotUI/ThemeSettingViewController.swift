...
class ThemeSettingViewController: UIViewController {
    ...
    @IBAction func handleCancelBarButtonPressed(_ sender: UIBarButtonItem) {
        Constants.APPDELEGATE?.setThemeConfiguration()
    ...
    @IBAction func handlePreviewBarButtonPressed(_ sender: UIBarButtonItem) {
        ...
        let accentColorRangeSetting = self.themeSettings[self.accentColorRangeCollectionView]!
        let accentColorRangeSelectedIndex = accentColorRangeSetting.selectedIndex
        if let accentColor = accentColorRangeSetting.accentColors {
            let themeConfiguration = UIDThemeConfiguration(colorRange: colorRangeSetting.colorRanges[colorRangeSelectedIndex],
                                                           ...
                                                           accentColorRange: accentColor[accentColorRangeSelectedIndex])
        ...
        self.forceRefreshUIApperence()
        self.performSegue(withIdentifier: Constants.THEME_PREVIEW_STORYBOARD_SEGUE, sender: sender)
    }
...
UICollectionViewDelegateFlowLayout {
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        if self.themeSettings[collectionView]!.selectedIndex != indexPath.row {
            ...
            case self.colorRangeCollectionView:
                if let accentCollectionViewSetting = themeSettings[accentColorRangeCollectionView] {
                    if let oldAccentColors = accentCollectionViewSetting.accentColors, oldAccentColors.count > 0 {
                        var aSetting = themeSettings[accentColorRangeCollectionView]!
                        configureAccent(for: &aSetting,
                                        colorRange: oldAccentColors[accentCollectionViewSetting.selectedIndex])
                    } else {
                        configureAccent(for: &themeSettings[accentColorRangeCollectionView]!,
                                        colorRange: UIDThemeManager.sharedInstance.defaultTheme?.accentColorRange)
                    }
                }
            default:
                break
    ...
    private func selectedColorRangeForCollectionView(_ collectionView: UICollectionView,
        ...
        case self.accentColorRangeCollectionView:
            if let accentColors = collectionViewSetting.accentColors {
                selectedColorRange = accentColors[indexPath.row]
            }
        default:
    ...
    fileprivate func configureAccent(for accentCollectionViewSetting: inout CollectionViewSetting,
                                     colorRange: UIDColorRange?) {
        settingForColorRangeCollectionView(collectionViewSetting: &accentCollectionViewSetting, isForAccent: true)
        
        if let indexOfSelectedAccent = accentCollectionViewSetting.accentColors?.firstIndex(where: { $0 == colorRange }) {
            accentCollectionViewSetting.selectedIndex = indexOfSelectedAccent
        } else {
            accentCollectionViewSetting.selectedIndex = 0
        }
    }
}
...
extension ThemeSettingViewController {
    fileprivate func settingForCollectionView(_ collectionView: UICollectionView) -> CollectionViewSetting {
        ...
        switch collectionView {
        case self.colorRangeCollectionView:
            self.settingForColorRangeCollectionView(collectionViewSetting: &collectionViewSetting, isForAccent: false)
        ...
        case self.accentColorRangeCollectionView:
            configureAccent(for: &collectionViewSetting,
                            colorRange: UIDThemeManager.sharedInstance.defaultTheme?.accentColorRange)
        default:
    ...
    fileprivate func settingForColorRangeCollectionView(collectionViewSetting: inout CollectionViewSetting,
                                                        isForAccent: Bool) {
        var actualColorRanges = collectionViewSetting.colorRanges
        
        if isForAccent {
            if let selectedColorRangeSetting = self.themeSettings[self.colorRangeCollectionView] {
                let selectedColorRange = selectedColorRangeSetting.colorRanges[selectedColorRangeSetting.selectedIndex]
                let supportedAccentColors = UIDTheme.supportedAccentColors(for: selectedColorRange)
                collectionViewSetting.accentColors = supportedAccentColors
                actualColorRanges = supportedAccentColors
            }
        }
        
        collectionViewSetting.numberOfItem = actualColorRanges.count
        collectionViewSetting.textArray.removeAll()
        collectionViewSetting.checkMarkIconColors.removeAll()
        collectionViewSetting.gradientColorLevels.removeAll()
        
        for color in actualColorRanges {
            collectionViewSetting.textArray.append(color.colorDisplayName())
            collectionViewSetting.checkMarkIconColors.append(color.checkMarkIconColors())
            collectionViewSetting.gradientColorLevels.append(color.gradientColorLevels())
        }
    }
