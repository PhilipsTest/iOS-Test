//
//  CCBResponseViewController.swift
...
class CCBResponseViewController : UIViewController,CCBMessageListner {
    ...
    private func showWSStatus() {
        DispatchQueue.main.async {
            let statusDict = [0:"Connecting",1:"Disconnected",3:"Connected"]
    ...
    private func showConversationDetails() {
        DispatchQueue.main.async {
            let  message = "\(CCBManager.shared.value(forKeyPath: "conversation.conversationID") ?? "") \n \(CCBManager.shared.value(forKeyPath: "conversation.token") ?? "token") \n \(CCBManager.shared.value(forKeyPath: "conversation.conversationToken") ?? "conv token") \n \(CCBManager.shared.value(forKeyPath: "conversation.webSocketURL") ?? "WS url")"
            self.responseTextView.text = self.responseTextView.text + "Conversation details present \(message) \n"
        }
    }

}

extension CCBResponseViewController {
